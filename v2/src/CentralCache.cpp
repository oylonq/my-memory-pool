...
void *CentralCache::fetchRange(size_t index) {
  // 索引检查，当索引大于等于 FREE_LIST_SIZE 时， 说明申请内存过大直接向系统申请
  if (index >= FREE_LIST_SIZE)
    return nullptr;

  // 自旋锁保护
  while (locks_[index].test_and_set(std::memory_order_acquire)) {
    std::this_thread::yield(); // 添加线程让步，避免忙等待，避免过度消耗 CPU
  }
  ...
  try {
    ...
    if (!result) {
      ...
      if (blockNum > 1) {
        ...
        *reinterpret_cast<void **>(result) = nullptr;
        ...
        if (trackerIndex < spanTrackers_.size()) {
          ...
          // 第一个块 result 已被分配出去，所以初始空闲块数为 blockNum - 1
          spanTrackers_[trackerIndex].freeCount.store(
              blockNum - 1, std::memory_order_release);
        }
      }
    } else {
      // 保存 result 的下一个节点
      ...
      *reinterpret_cast<void **>(result) = nullptr;
      ...
      if (tracker) {
        // 减少一个空闲块
        tracker->freeCount.fetch_add(1, std::memory_order_release);
      }
    }
  } catch (...) {
    locks_[index].clear(std::memory_order_release);
